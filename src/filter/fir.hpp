...
public:
    ...
    FIR(std::vector<double>& iResponse, Source* src) 
        : Filter(src), kern(iResponse), sbuf(iResponse.size(), 0), bufId(0) {}
    FIR(Source* src) : Filter(src), kern(), sbuf(), bufId(0){}
...
class PassFilter : public FIR {
    ...
    size_t segments = 200;
public:
    PassFilter(std::vector<double> dp, Source* src = nullptr);  
    PassFilter(Interpolated<double>& dp, int srate, Source* src = nullptr, size_t scount = 200);
    PassFilter(std::istream& str, const int srate, const MakeFlags& flags = MakeFlags::all);
    PassFilter(const PassFilter& f) = default;
    std::unique_ptr<Source> copy() {return std::make_unique<PassFilter>(*this);}
